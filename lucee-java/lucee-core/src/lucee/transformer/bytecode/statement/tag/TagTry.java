...
public final class TagTry extends TagBase implements FlowControlRetry {
	...
	public void _writeOut(BytecodeContext bc) throws BytecodeException {
		...
		while(it.hasNext()) {
			...
			if(stat instanceof Tag) {
				tag=(Tag) stat;
				if(tag.getTagLibTag().getTagClassName().equals("lucee.runtime.tag.Catch"))	{
					catches.add(tag);
					continue;
				}
				else if(tag.getTagLibTag().getTagClassName().equals("lucee.runtime.tag.Finally"))	{
					tmpFinal=tag;
					continue;
				}
			}
	...
	private boolean hasFinally(){
		...
		while(it.hasNext()) {
			...
			if(stat instanceof Tag) {
				tag=(Tag) stat;
				if(tag.getTagLibTag().getTagClassName().equals("lucee.runtime.tag.Finally"))	{
					return true;
				}
			}
		}
